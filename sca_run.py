...
class sca_run:
	...
	def set_filter_params(self, min_cells=0, min_genes=500, max_genes=7000, max_counts=30000, min_counts=0, max_mito=1):
		self.__set_param_attr(locals())

	## Function to set multiple analysis parameters in attributes
	def set_analysis_params(self, n_neighbors=30, n_pcs=15, spread=1, min_dist=0.4, resolution=0.6, remove_batch_effects=False, do_tSNE=False, cell_score_lists=[], phate=False):
		self.__set_param_attr(locals())
	...
	def __create_scanpy_anndata(self,storage_mount_point, sampleID, annotation_dict):
		...
		# annotation_dict[sampleID][0] = annotation_dict[sampleID][0].replace('filtered_feature_bc_matrix.h5','cleaned.h5')
		newAdata = sc.read_10x_h5(''.join([storage_mount_point, annotation_dict[sampleID][0]]))#, genome='hg19')# or genome='GRCh38'
		if newAdata.var_names[0].startswith('hg19_'):
			newAdata.var_names = [index[5:] for index in newAdata.var_names.values]
	...
	def __count_cluster_scores(self, adata, cell_score_list):
		cluster_scores = {}
		for cluster in adata.obs['sampleName'].cat.categories:
			cluster_scores[cluster] = adata[adata.obs['sampleName'].isin([cluster])].obs[cell_score_list].mean()
	...
	def filter_data(self, adata, min_cells=None, min_genes=None, max_counts=None, min_counts=None, max_genes=None, max_mito=None,
		...
		adata = adata[((adata.obs['n_genes'] < param_dict['max_genes'])   # Keep cells with less than __ genes to remove most doublets
					& (adata.obs['n_counts'] < param_dict['max_counts'])
					& (adata.obs['n_counts'] > param_dict['min_counts'])   # Keep cells with less than __ UMIs to catch a few remaining doublets
	...
	def preprocess_data(self,adata):
		...
		if self.phate:
			sce.tl.phate(adata, knn=15, t=20, gamma=0)
			print(adata.obsm['X_phate'])
			...
			sce.pl.phate(adata,save='_phate_sampleName.png', color='sampleName', size=10, show=False, palette=colors)
			sce.pl.phate(adata,save='_phate_age.png', color='age', size=10, show=False, palette=colors)
			sce.pl.phate(adata,save='_phate_preprocessing.png', color=['n_genes','n_counts','percent_mito'],size=10,show=False)
		print(self.cell_score_lists)
	...
	def run_analysis(self,adata, n_neighbors=None, n_pcs=None, spread=None,
		...
		sc.tl.umap(adata, spread=param_dict['spread'], min_dist=param_dict['min_dist'])#, init_pos='paga')#, n_components=50) # Min_dist needs to be between 0.01 to 0.5
	...
	def plot_sca(self, adata, figdir='./figures/', adata_preFiltered=None,
		...
		if self.cell_score_lists:
			...
			sc.pl.violin(adata,self.cell_score_lists, groupby='sampleName', size=0.1,
		...
		if self.phate:
			sce.pl.phate(adata,save='_phate.png', show=False)
	...
	def pipe_ext(self, analysis_params_ext, adata_loaded=None, figdir='./figures/', extracted=None, load_save=None, new_save='adata_save.p', final_quality=False, label='',
				 preprocess=True):
		'''
		...
		if not np.any([not cluster.isdigit() for cluster in extracted]):
			...
			if load_save:
				...
				run_save = self.pipe_basic(figdir,adata_loaded=adata_loaded)
